...
exports.findOne = async(req, res) => {
    ...
    console.log("Find post wiht id ", id);
...
exports.create = async(req, res) => {
    ...
    try {
        ...
        res.status(200).json({ status: true, data: result })
        console.log("Success in inserting post");
    } catch (err) {
        res.status(400).json({ status: false, data: err });
...
exports.deleteCategories = async(req, res) => {
    const id = req.params.id
    console.log("Remove post categories");

    try {
        const result = await postService.deleteCategories(req.body);
        res.status(200).json({ status: true, data: result })
        console.log("Success in removing post categories");
    } catch (err) {
        res.status(400).json({ status: false, data: err });
        console.log("Problem in removing post categories")
    }
}